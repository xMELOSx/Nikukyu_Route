 ...
 * older version of this function (which used an incorrect biweekly phase),
 * it is discarded and recomputed so the next reset always lands on the
 * correct biweekly Monday at 5:00 AM.
 */
export function checkAutoReset(state: PlayDataState, now: number = Date.now()): PlayDataState {
  ...
  if (!periodStart) {
    ...
    const expectedStart = getInitialPeriodStart(now);

        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
          ...
          <div style={{ display: 'flex', gap: '3px' }}>
            <button className="btn-cyber" style={{ padding: '1px 5px', fontSize: '9px', clipPath: 'none', borderColor: '#0f0', color: '#0f0' }}
              onClick={() => { const c = routeApi.route.hiddenMarkerTypes || []; const t = ['eh','rare','cardkey','vault','boss','gbattle','gpicking','glong_picking','phone','warp','stairs','info','note','text','drawer','shelf']; const n = c.filter(x => !t.includes(x)); if (n.length===c.length) return; routeApi.setRoute(p => ({...p, hiddenMarkerTypes: n})); }}>ALL ON</button>
            <button className="btn-cyber" style={{ padding: '1px 5px', fontSize: '9px', clipPath: 'none', borderColor: '#f55', color: '#f55' }}
              onClick={() => { const c = routeApi.route.hiddenMarkerTypes || []; const t = ['eh','rare','cardkey','vault','boss','gbattle','gpicking','glong_picking','phone','warp','stairs','info','note','text','drawer','shelf']; const a = t.filter(x => !c.includes(x)); if (a.length===0) return; const n = [...new Set([...c,...a])]; routeApi.setRoute(p => ({...p, hiddenMarkerTypes: n})); }}>ALL OFF</button>
          </div>
        </div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px', marginBottom: '8px' }}>
          {(['eh','rare','cardkey','vault','boss','gbattle','gpicking','glong_picking','phone','warp','stairs','info','note','text','drawer','shelf'] as MarkerType[]).map(t2 => {
